# 함수란? 코드의 특정 블럭을 하나의 블럭으로 뭈는 것. 코드의 재사용성을 높임, 가독성 향상
# 함수는 매개변수를 가질수 있고, 반환값을 가질수 있음
# 식별자는 스네이크 표기법으로 작성하고, 식별자뒤에 () 소괄호사 있음
# 일반적으로 매개변수와 함수의 호출하는 인자의 갯수가 일치해야 함
# def 키워드를 사용해서 정의

# 함수의 반복호출, 매개변수는 존재하고 반환값은 함수 정의
...
name_card("장어녀","서울시 강남구 삼성동", "010-1234-9999","센터")
...
def withdraw(balance, out_val): # 잔액과 출금을 매개변수로 전달 받음
    if balance >= out_val:
        ...
        print(f"출금이 실패 하였습니다. 잔액은 {balance}입니다.")
        return balance
# # 조용민
# balance = 0 # 외부에서 선언한 변수
# name = input("계좌 개설할 이름을 입력 : ")
# open_account(name) # 인자값으로 전달한 이름을 반환값으로 되돌려 받음
# balance = deposit(balance, 1000) # 외부에서 선언한 인자값으로 전달, 입금액을 인자값으로 전달
...
# 매개변수에 기본값이 정의 되어 있는 경우 호출 시 인자값을 넣지 안으면 기본값으로 호출
def profile(name, grade = 2, age = 18, school = "태양고") :
    print(f"이름 : {name}, 학교 : {school}, 학년 : {grade}, 나이 : {age}")

# profile("나희도")
# profile("고유림")
# # profile() # name은 기본값 인자가 아니므로 반드시 인자값을 전달해야함
# profile("백이진", None)

# 가변 매개 변수 : 함수의 매개변수 앞에 *(별표)를 붙히면 매개 변수가 인자값으로 몇개를 전달하든 튜플로 인식해서 처리 가능
...
# 지역 변수, 전역 변수
# 대부분은 언어는 블록스코프를 기반으로 변수의 생명주기를 관리
...
def game2(k, player):
    k -= player  # knife = knife - player
    print(f"남아 있는 칼은 {k} 자루 입니다.")
    return k
...
h = int(input("키 입력 : "))
g = int(input("남성 [1]남성 / [2]여성 : "))
weight = round(std_weight(h,g),2) # 소수점 3번째 자리를 반올림해서 소수점 2저리 만듬
gender_str = "", "남성", "여성"
print(f" 키 : {h}cm {g}의 표준 체중은 {weight}kg 입니다.")
